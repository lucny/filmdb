...
class Film(models.Model):
    ...
    title = models.CharField(
        ...
        # Nastavení dvou validačních podmínek - minimální délka řetězce a kontrola podle regex
        validators=[
            MinLengthValidator(2, 'Název nesmí být kratší než 2 znaky!'),
            RegexValidator(r'^[^+\-*/%&;:.,]', 'Název nesmí začínat speciálním znakem!')
        ]
    )
    ...
    runtime = models.PositiveSmallIntegerField(
        ...
        # Nastavení číselného validátoru
        validators=[
            MaxValueValidator(1000, 'Hodnota nesmí být větší než 1000!'),
        ]
    )
    ...
    rating = models.FloatField(
        ...
        # Nastavení číselných validátorů
        validators=[
            MinValueValidator(0, 'Hodnota nesmí být menší než 0!'),
            MaxValueValidator(10, 'Hodnota nesmí být větší než 10!'),
        ]
    )
    ...
    def clean(self):
        '''
        Metoda clean() v modelu Django slouží k validaci a upravě dat před uložením do databáze.
        Je volána automaticky při volání metody save() na instanci modelu.
        '''
        # První validační podmínka kontroluje, zda hodnota year v release_date je menší než 1895.
        # Pokud je tato podmínka splněna, vyvolá se výjimka ValidationError s příslušnou chybovou zprávou.
        # Tím se zamezí uložení filmu s neplatným datem uvedení.
        if (self.release_date.year < 1895):
            raise ValidationError({'release_date': 'Datum uvedení filmu nemůže být před rokem 1895!'})
        # Druhá podmínka kontroluje, zda hodnota release_date je větší než aktuální datum (datetime.date.today()).
        # Pokud je tato podmínka splněna, opět se vyvolá výjimka ValidationError s příslušnou chybovou zprávou,
        # aby se zabránilo uložení filmu s datem uvedení v budoucnosti.
        if (self.release_date > datetime.date.today()):
            raise ValidationError({'release_date': 'Datum uvedení filmu nemůže být v budoucnosti!'})
